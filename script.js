document.addEventListener("DOMContentLoaded", function () {
        ...
        const heroObserver = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (heroText)  Object.assign(heroText.style,  { opacity: "0", transform: "translateX(-50px)", transition: "none" });
                    if (heroImage) Object.assign(heroImage.style, { opacity: "0", transform: "translateX(50px)",  transition: "none" });
                }
            });
        }, { threshold: 0.2 });
    ...
    function runStatCounters() {
        ...
        statCards.forEach((card, i) => {
            ...
            if (!h3 || !cfg) return;
            if (cfg.target === null) { h3.textContent = cfg.text; return; }
            const delay = i * 250;
            h3.textContent = "0" + cfg.suffix;
            setTimeout(() => animateCounter(h3, cfg.target, cfg.suffix, cfg.duration), delay);
        });
    }
    ...
    function resetStatCounters() {
        ...
        statCards.forEach((card, i) => {
            ...
            const cfg = statConfigs[i];
            if (!h3 || !cfg) return;
            h3.textContent = cfg.target === null ? cfg.text : "0" + cfg.suffix;
        });
    }
        ...
        const aboutObserver = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (aboutText)  Object.assign(aboutText.style,  { opacity: "0", transform: "translateX(-60px)", transition: "none" });
                    if (aboutStats) Object.assign(aboutStats.style, { opacity: "0", transform: "translateX(60px)",  transition: "none" });
                    document.querySelectorAll(".stat-card").forEach(card => {
                        Object.assign(card.style, { opacity: "0", transform: "translateY(40px) scale(0.95)", transition: "none" });
                    });
                    resetStatCounters();
                }
            });
    ...
    (function initSkills() {
        ...
        if (skillCanvas) {
            ...
            function sResize() {
                ...
                for (let i = 0; i < n; i++) {
                    pts.push({
                        ...
                        r: Math.random()*1.8+0.5, a: Math.random()*0.3+0.08,
            ...
            function sDraw(t) {
                ...
                pts.forEach(p => {
                    p.x=(p.x+p.vx+sW)%sW; p.y=(p.y+p.vy+sH)%sH;
                    const pulse=Math.sin(t*0.0009+p.x*0.02)*0.06;
                    sCtx.beginPath();sCtx.arc(p.x,p.y,p.r,0,Math.PI*2);
                    sCtx.fillStyle=`hsla(${p.hue},75%,72%,${p.a+pulse})`;sCtx.fill();
                });
                for (let i=0;i<pts.length;i++) {
                    for (let j=i+1;j<pts.length;j++) {
                        const dx=pts[i].x-pts[j].x,dy=pts[i].y-pts[j].y,d=Math.sqrt(dx*dx+dy*dy);
                        if(d<95){sCtx.beginPath();sCtx.moveTo(pts[i].x,pts[i].y);sCtx.lineTo(pts[j].x,pts[j].y);sCtx.strokeStyle=`rgba(167,139,250,${(1-d/95)*0.07})`;sCtx.lineWidth=0.5;sCtx.stroke();}
                    }
                }
            ...
            const skillBgObs=new IntersectionObserver(entries=>{
                if(entries[0].isIntersecting){cancelAnimationFrame(sRaf);sRaf=requestAnimationFrame(sDraw);}
                else cancelAnimationFrame(sRaf);
            },{threshold:0.01});
        ...
        function fillCircles() {
            document.querySelectorAll(".circle-fill").forEach((ring,i)=>{
                ...
                ring.style.transition="none";ring.style.strokeDashoffset=CIRCUM;
            ...
            document.querySelectorAll(".circle-percent").forEach((el,i)=>{
                ...
                setTimeout(()=>{
                    const timer=setInterval(()=>{
                        current=Math.min(current+2,target);el.textContent=current+"%";
        ...
        function resetCounters() {
            document.querySelectorAll(".circle-percent").forEach(el=>{ el.textContent="0%"; });
        }
        ...
        skillCards.forEach(card=>{
            ...
            card.addEventListener("mousemove",e=>{
                ...
                rafTilt=requestAnimationFrame(()=>{
                    ...
                    if(shine){const px=((e.clientX-bounds.left)/bounds.width)*100,py=((e.clientY-bounds.top)/bounds.height)*100;shine.style.background=`radial-gradient(380px circle at ${px}% ${py}%,rgba(255,255,255,0.09) 0%,transparent 65%)`;}
                });
            });
        ...
        function resetSkillCards() {
            skillCards.forEach(card=>{card.style.transition="none";card.style.opacity="0";card.style.transform="translateY(45px) scale(0.95)";});
        }
        ...
        const skillsObserver=new IntersectionObserver(entries=>{
            entries.forEach(entry=>{
                if(entry.isIntersecting){animateSkillCards();setTimeout(()=>{fillCircles();runCounters();},250);}
                else{resetSkillCards();resetCircles();resetCounters();}
            });
        },{threshold:0.12});

        skillsObserver.observe(skillsSection);
        resetSkillCards();resetCircles();resetCounters();
    ...
    (function initProjects() {
        ...
        if(projCanvas){
            ...
            let pW,pH,pRaf;const dots=[];
            const COLS=22,ROWS=14;

            function pResize(){
                pW=projCanvas.width=projCanvas.offsetWidth;pH=projCanvas.height=projCanvas.offsetHeight;
                dots.length=0;const colGap=pW/COLS,rowGap=pH/ROWS;
                for(let r=0;r<=ROWS;r++)for(let c=0;c<=COLS;c++)
                    dots.push({x:c*colGap,y:r*rowGap,ox:c*colGap,oy:r*rowGap,
                        r:Math.random()*1.4+0.5,alpha:Math.random()*0.4+0.1,
                        speed:Math.random()*0.008+0.004,phase:Math.random()*Math.PI*2});
            }
            ...
            function pDraw(t){
                ...
                dots.forEach(d=>{
                    ...
                    pCtx.fillStyle=`rgba(148,163,184,${d.alpha})`;pCtx.fill();
                });
                for(let i=0;i<dots.length;i++)for(let j=i+1;j<dots.length;j++){
                    const dx=dots[i].x-dots[j].x,dy=dots[i].y-dots[j].y,dist=Math.sqrt(dx*dx+dy*dy);
                    if(dist<90){pCtx.beginPath();pCtx.moveTo(dots[i].x,dots[i].y);pCtx.lineTo(dots[j].x,dots[j].y);
                        pCtx.strokeStyle=`rgba(148,163,184,${(1-dist/90)*0.08})`;pCtx.lineWidth=0.5;pCtx.stroke();}
                }
                pRaf=requestAnimationFrame(pDraw);
            }

            const projBgObs=new IntersectionObserver(entries=>{
                if(entries[0].isIntersecting){cancelAnimationFrame(pRaf);pRaf=requestAnimationFrame(pDraw);}
                else cancelAnimationFrame(pRaf);
            },{threshold:0.01});
        ...
        projectCards.forEach(card=>{
            ...
            card.addEventListener("mousemove",e=>{
                ...
                rafTilt=requestAnimationFrame(()=>{
                    ...
                    const dx=(e.clientX-cx)/(bounds.width/2),dy=(e.clientY-cy)/(bounds.height/2);
                    inner.style.transition="box-shadow 0.4s ease,border-color 0.4s ease";
                    inner.style.transform=`perspective(1000px) rotateX(${-dy*10}deg) rotateY(${dx*10}deg) translateZ(8px)`;
                    if(shine){const px=((e.clientX-bounds.left)/bounds.width)*100,py=((e.clientY-bounds.top)/bounds.height)*100;shine.style.background=`radial-gradient(400px circle at ${px}% ${py}%,rgba(255,255,255,0.09) 0%,transparent 60%)`;}
                });
            });
            card.addEventListener("mouseleave",()=>{
                cancelAnimationFrame(rafTilt);
        ...
        function resetProjectCards(){projectCards.forEach(card=>{card.style.transition="none";card.style.opacity="0";card.style.transform="translateY(50px) scale(0.96)";});}
        function animateProjectCards(){projectCards.forEach((card,i)=>{setTimeout(()=>{card.style.transition="opacity 0.65s ease,transform 0.65s cubic-bezier(0.34,1.2,0.64,1)";card.style.opacity="1";card.style.transform="translateY(0) scale(1)";},i*120);});}

        const projectObserver=new IntersectionObserver(entries=>{
            entries.forEach(entry=>{
                if(entry.isIntersecting)animateProjectCards();else resetProjectCards();
            });
        },{threshold:0.1});
    ...
    // ============================================================
    // 8. CONTACT SECTION — Animated Background
    // ============================================================
    ...
    (function initContactBackground() {
        ...
        const grid=document.createElement("div");grid.classList.add("contact-grid");
        contactSection.insertBefore(grid,contactSection.firstChild);

        const canvas=document.createElement("canvas");canvas.id="contactBgCanvas";
        contactSection.insertBefore(canvas,contactSection.firstChild);
        const ctx=canvas.getContext("2d");
        let W,H,rafId;const PARTICLE_COUNT=55,particles=[];

        function resize(){
            W=canvas.width=contactSection.offsetWidth;H=canvas.height=contactSection.offsetHeight||600;
            particles.length=0;
            for(let i=0;i<PARTICLE_COUNT;i++)particles.push({x:Math.random()*W,y:Math.random()*H,vx:(Math.random()-0.5)*0.35,vy:(Math.random()-0.5)*0.35,r:Math.random()*1.8+0.4,alpha:Math.random()*0.35+0.08,hue:Math.random()>0.6?200:(Math.random()>0.5?240:270)});
        }
        ...
        function drawBg(t){
            ...
            particles.forEach(p=>{
                ...
                ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle=`hsla(${p.hue},80%,70%,${p.alpha+pulse})`;ctx.fill();
            });
            for(let i=0;i<particles.length;i++)for(let j=i+1;j<particles.length;j++){
                const dx=particles[i].x-particles[j].x,dy=particles[i].y-particles[j].y,dist=Math.sqrt(dx*dx+dy*dy);
                if(dist<110){ctx.beginPath();ctx.moveTo(particles[i].x,particles[i].y);ctx.lineTo(particles[j].x,particles[j].y);ctx.strokeStyle=`rgba(56,189,248,${(1-dist/110)*0.06})`;ctx.lineWidth=0.5;ctx.stroke();}
            }
            rafId=requestAnimationFrame(drawBg);
        }

        const bgObs=new IntersectionObserver(entries=>{
            if(entries[0].isIntersecting){cancelAnimationFrame(rafId);rafId=requestAnimationFrame(drawBg);}
    ...
    (function initFooter() {
        ...
        [footerInner,footerBottom].forEach((el,i)=>{
            if(!el)return;
        ...
        const footerObs=new IntersectionObserver(entries=>{
            entries.forEach(entry=>{
                if(entry.isIntersecting){
                    ...
                    [footerInner,footerBottom].forEach(el=>{
                        if(!el)return;
    ...
    if (typingElement) {
        ...
        function typeEffect(){
            ...
            if(!isDeleting){
                ...
                charIndex++;
                if(charIndex===currentRole.length){setTimeout(()=>{isDeleting=true;typeEffect();},1200);return;}
            }else{
                typingElement.textContent=currentRole.substring(0,charIndex-1);
                charIndex--;
